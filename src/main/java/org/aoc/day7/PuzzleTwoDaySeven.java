...
public class PuzzleTwoDaySeven {
    ...
    private void checkSumFromCombinations(Long answer, Long current, List<Long> input, int i) {
        ...
        checkSumFromCombinations(answer, current * input.get(i), input, i+1);
        long numberAfterJoinOperator = Long.parseLong(String.valueOf(current).concat(String.valueOf(input.get(i))));
        checkSumFromCombinations(answer, numberAfterJoinOperator, input, i+1);
    }
}
